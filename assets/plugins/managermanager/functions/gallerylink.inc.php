		...
		foreach ($fields as $field) {
			...
			switch ($field) {
				...
				if (isset($mm_fields[$field]))  { // Check the fields exist,  so we're not writing JS for elements that don't exist
                    ...
                    $output .= '    var galleryLink = $j(\'<a href="'.$modx->config['site_url'].'manager/index.php?a=112&id='.$moduleid.'&action=view&content_id=\'+pid'.$mm_fields[$field]['fieldname'].'+\'">Manage Photos</a>\').insertAfter(gl'.$mm_fields[$field]['fieldname'].');'."\n";
                	$output .= '} else {'."\n";
