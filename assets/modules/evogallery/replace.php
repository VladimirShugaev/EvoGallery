...
if (is_uploaded_file($_FILES['Filedata']['tmp_name'])){
	...
	$target_file = $target_dir . $params['imagePrefix'] . $_FILES['Filedata']['name'];
	$target_thumb = $target_dir . $params['thumbsPath'] . $params['thumbPrefix'] . $_FILES['Filedata']['name'];
	...
	if (!file_exists($target_dir . $params['thumbsPath']))
	{
		mkdir($target_dir . $params['thumbsPath'], 0777);
	}
    ...
    if($filename !== $_FILES['Filedata']['name']){
		if (file_exists($target_dir . $params['thumbsPath'] . $params['thumbPrefix'] .  $filename))
			unlink($target_dir . $params['thumbsPath'] . $params['thumbPrefix'] .  $filename);
		if (file_exists($target_dir . $params['imagePrefix'] . $filename))
			unlink($target_dir . $params['imagePrefix'] . $filename);
    }
