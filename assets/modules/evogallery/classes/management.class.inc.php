...
class GalleryManagement
{
	...
	*/
	...
	{
		global $modx;		
		$this->lang = $this->getLang($modx);

		$this->config = $params;
		
		if($this->config['debug']) $this->debug .= "Gallery Manager : Debug<br />";
		if($this->config['debug']) $this->debug .= "|-- Init Construtor: ";

		if ($this->lang['EG_language'] !='') $prefixlang  = $this->lang['EG_language']  . '.';

		$this->mainTemplate = $prefixlang . 'template.html.tpl';
		...
		if($this->config['debug']) $this->debug .= "Ok<br />";
	}
	...
	*/
	function execute()
	{
		...
		$tpl = str_replace('[+module_title+]', stripslashes($this->lang['EG_module_title']), $tpl);			
		$tpl = str_replace('[+debug+]', $this->debug, $tpl);
	...
	*/
	function editImage()
	{
		...
		if($this->config['debug']) $this->debug .= "|-- editImage:<br />";
		...
		$tpl = str_replace('[+img_title+]', stripslashes($this->lang['EG_img_title']), $tpl);
		$tpl = str_replace('[+img_description+]', stripslashes($this->lang['EG_img_description']), $tpl);
		$tpl = str_replace('[+img_keyword+]', stripslashes($this->lang['EG_img_keyword']), $tpl);
		$tpl = str_replace('[+img_update+]', stripslashes($this->lang['EG_img_update']), $tpl);
		$tpl = str_replace('[+file_upload+]', stripslashes($this->lang['EG_file_upload']), $tpl);	
		$tpl = str_replace('[+btn_update+]', stripslashes($this->lang['EG_btn_update']), $tpl);		
	...
	*/
	function viewListing()
	{
		...
		if($this->config['debug']) $this->debug .= "|-- Function viewListing:<br />";
		$this_page = $this->current . '?a=' . $this->a . '&id=' . $this->id;

		$tpl = file_get_contents($this->config['modulePath'] . 'templates/' . $this->listingTemplate);
		if($this->config['debug']) $this->debug .= "|     |-- Template: ".$this->listingTemplate."<br />";
		...
		if (isset($_GET['query']))
		{
			...
			$filter .= ")";
			$header = $this->header($this->lang['EG_search_result']);
		}
		else
		{
		...
		if ($ds = $modx->db->query($query))
		{
			...
			{
				$documents[] = array(
					'pagetitle' => '<a href="' . $this_page . '&action=view&content_id=' . $row['id'] . '" title="' . $this->lang['EG_link_doc'] . '">' . $row['pagetitle'] . ' (' . $row['id'] . ')</a>',
		...
		{
			if($this->config['debug']) $this->debug .= "|     |-- Documents<br />";
			
			// Create the table header definition with each header providing a link to sort by that field
			$documentTableHeader = array(
				'pagetitle' => $table->prepareOrderByLink('c.pagetitle', $this->lang['EG_table_title']),
				'longtitle' => $table->prepareOrderByLink('c.longtitle', $this->lang['EG_table_longtitle']),
				'photos' => $table->prepareOrderByLink('photos', $this->lang['EG_table_photo']),
				'editedon' => $table->prepareOrderByLink('c.editedon', $this->lang['EG_table_lastedited']),
			);
		...
		{
			$table_html = '<p>' . $this->lang['EG_no_record'] . '</p>';  // No records were found
		}
		else
		{
			$table_html = '<p class="first">' . $this->lang['EG_no_children'] . '</p>';
		}
	...
	*/
	function viewGallery()
	{
		...
		if($this->config['debug']) $this->debug .= "|-- Function viewGallery:<br />";
		$this_page = $this->current . '?a=' . $this->a . '&id=' . $this->id;
		...
		if ($modx->db->getRecordCount($result) > 0)
		{
			...
			elseif (isset($_GET['delete']))  // Delete requested image
			{
                		...
                		if ($modx->db->getRecordCount($result) > 0)
				{
					...
					if (file_exists($target_dir . $this->config['thumbsPath'] . $this->config['thumbPrefix'] . $filename))
						unlink($target_dir . $this->config['thumbsPath'] . $this->config['thumbPrefix'] . $filename);
					if (file_exists($target_dir . $this->config['imagePrefix'] . $filename))
						unlink($target_dir . $this->config['imagePrefix'] . $filename);
			...
			if ($this->lang['EG_language'] !='') $prefixlang  = $this->lang['EG_language']  . '.';
			$upload_script = file_get_contents($this->config['modulePath'] . 'templates/' . $prefixlang  . 'upload.js.tpl');
...
//				$thumbs .= "<li><div class=\"thbButtons\"><a href=\"" . $this_page . "&action=edit&content_id=$content_id&edit=" . urlencode($row['filename']) . "\" title=\"" . stripslashes($row['filename']) . "\" class=\"edit\" rel=\"moodalbox 420 375\">Edit</a><a href=\"$this_page&action=view&content_id=$content_id&delete=" . urlencode($row['filename']) . "\" onclick=\"return Uploader.deleteConfirm()\" class=\"delete\">Delete</a></div><img src=\"" . $this->thumbHandler . "content_id=" . $content_id . "&filename=" . urlencode($row['filename']) . "\" alt=\"" . htmlentities(stripslashes($row['filename'])) . "\" class=\"thb\" /><input type=\"hidden\" name=\"sort[]\" value=\"" . urlencode($row['filename']) . "\" /></li>\n";
			...
			$tpl = str_replace('[+upd_title+]', stripslashes($this->lang['EG_upd_title']), $tpl);
			$tpl = str_replace('[+upd_tip+]', stripslashes($this->lang['EG_upd_tip']), $tpl);
			$tpl = str_replace('[+upd_info+]', stripslashes($this->lang['EG_upd_info']), $tpl);
			$tpl = str_replace('[+file_upload+]', stripslashes($this->lang['EG_file_upload']), $tpl);
			$tpl = str_replace('[+upd_clearqueue+]', stripslashes($this->lang['EG_upd_clearqueue']), $tpl);
			$tpl = str_replace('[+upd_goback+]', stripslashes($this->lang['EG_upd_goback']), $tpl);
			$tpl = str_replace('[+upd_saveorder+]', stripslashes($this->lang['EG_upd_saveorder']), $tpl);
			$tpl = str_replace('[+upd_gobackinfo+]', stripslashes($this->lang['EG_upd_gobackinfo']), $tpl);
			$tpl = str_replace('[+upd_saveorderinfo+]', stripslashes($this->lang['EG_upd_saveorderinfo']), $tpl);			
			$tpl = str_replace('[+upd_sorting+]', stripslashes($this->lang['EG_upd_sorting']), $tpl);	
			$tpl = str_replace('[+upd_sortinfo+]', stripslashes($this->lang['EG_upd_sortinfo']), $tpl);		
	...
	*/
	function header($title = '')
	{
		...
		if (isset($_GET['query']))
			$search = '<label for="query">' . $this->lang['EG_search'] . '</label> <input type="text" name="query" id="query" value="' . $_GET['query'] . '" />';
		else
			$search = '<label for="query">' . $this->lang['EG_search'] . '</label> <input type="text" name="query" id="query" />';

		// Generate breadcrumbs
		$result = $modx->db->select('id, pagetitle, parent', $modx->getFullTableName('site_content'), 'id=' . $parentId);
		$row = $modx->fetchRow($result);
		$breadcrumbs = '<a href="' . $this_page . '&action=view&content_id=' . $row['id'] . '" title="' . $this->lang['EG_link_productcategories'] . '">' . stripslashes($row['pagetitle']) . '</a>';
		while ($row['id'] > $this->config['docId'])
		{
			...
			$breadcrumbs = '<a href="' . $this_page . '&action=view&content_id=' . $row['id'] . '" title="' . $this->lang['EG_link_productcategories'] . '">' . stripslashes($row['pagetitle']) . '</a> &raquo; ' . $breadcrumbs;
		}
	...
	* Language
	*/	
    function getLang(&$modx) {
		
    	$_lang = array();
    	$managerLanguage = $modx->config['manager_language'];
		
		include MODX_BASE_PATH.'assets/modules/evogallery/lang/english.inc.php';
		if($managerLanguage != 'english') {
			if (file_exists(MODX_BASE_PATH.'assets/modules/evogallery/lang/'.$managerLanguage.'.inc.php')) {
     			include MODX_BASE_PATH.'assets/modules/evogallery/lang/'.$managerLanguage.'.inc.php';
			}
		}
